...
│   ├── augment_main.ipynb                 # GAN training notebook (Colab)
│   ├── cnn_main.ipynb                     # CNN training notebook (Colab)
│   └── schizo_gan_data_cleaning.qmd       # Data cleaning and subject manifest
...
| `No_Aug_cm.png` | Confusion matrix for the no-augmentation baseline condition | `plot_confusion_matrix()` in `src/cnn_run.py` |
| `No_Aug_history.png` | Training loss and validation accuracy curves for the no-augmentation condition | `_plot_training_history()` in `src/cnn_run.py` |
| `results_table.png` | Summary table of all metrics across all 8 conditions | generated from `results/results_table.csv` via pandas/matplotlib |
| `demographics_table.png` | Subject demographics table (dataset, diagnosis counts, sites) | generated from subject manifest in `notebooks/schizo_gan_data_cleaning.qmd` |
| `sample_augments_DCGAN.png` | Example synthetic brain images generated by the DCGAN | saved from sample grid output during `dcgan_train()` in `src/augment.py` |
